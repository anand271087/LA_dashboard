...
export default function ProductTable({ data }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (minOrders)  rows = rows.filter(r => r.orderCount >= parseInt(minOrders, 10))
  ...
  }, [rowsAll, search, category, minRevenue, minOrders, sortCol, sortDir])

  const totalPages = Math.max(1, Math.ceil(filtered.length / PER_PAGE))
  const pageRows = filtered.slice((page - 1) * PER_PAGE, page * PER_PAGE)
  const activeFilters = [search, category, minRevenue, minOrders].filter(Boolean).length

  const clearFilters = () => { setSearch(''); setCategory(''); setMinRevenue(''); setMinOrders(''); setPage(1) }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input className="input-base pl-9" placeholder="Search products…"
        ...
        <div className="kpi-card grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input type="number" className="input-base mt-1" value={minRevenue}
              onChange={e => { setMinRevenue(e.target.value); setPage(1) }} placeholder="e.g. 5000"/>
          </div>
          <div>
            <label className="text-xs text-gray-600 font-medium">Min orders</label>
            <input type="number" className="input-base mt-1" value={minOrders}
              onChange={e => { setMinOrders(e.target.value); setPage(1) }} placeholder="e.g. 3"/>
          </div>
        </div>
      )}

      <p className="text-xs text-gray-500">
        Showing <span className="font-semibold text-gray-700">{filtered.length}</span> of {rowsAll.length} products
      </p>
          ...
          <table className="min-w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <Th label="Rev / Order" col="revenuePerOrder" sortCol={sortCol} sortDir={sortDir} onClick={toggleSort} right/>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {pageRows.map(r => (
                <tr key={r.key} className="hover:bg-gray-50">
                  <td className="td max-w-[300px] truncate" title={r.name}>{r.name}</td>
                  ...
                  <td className="td text-right">{fmt(r.revenuePerOrder)}</td>
                </tr>
              ))}
              {!pageRows.length && (
                <tr><td className="td text-center text-gray-500 py-8" colSpan={7}>No products match your filters</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {pageRows.map(r => (
          <div key={r.key} className="kpi-card">
            ...
            <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-gray-100 text-xs">
              ...
              <div><p className="text-gray-500">Rev/order</p><p className="font-semibold">{fmtK(r.revenuePerOrder)}</p></div>
            </div>
          </div>
        ))}
        {!pageRows.length && <p className="text-center text-gray-500 py-8 text-sm">No products match your filters</p>}
      </div>
